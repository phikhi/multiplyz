...
- `--world-surface` = surface **opaque** du thème → **scrim/casing de contraste** quand un fond-image réel du monde est affiché : sous le **titre** (story #189), autour du **bouton « Changer de joueur »** de bas d'écran (`FooterScrim`) et en **casing sous le trait du chemin** (`--map-node-path-casing-width`, story #202). Garantit la lisibilité de chacun indépendamment de la photo IA arbitraire.
