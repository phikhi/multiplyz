...
import {
  seedCanariProfile,
  CANARI_PROFILE_NAME,
  CANARI_PROFILE_AVATAR,
  CANARI_PROFILE_PIN,
} from "../e2e/seed-canari";
...
async function initialize() {
  ...
  if (
    !root.startsWith(realpathSync(tmpdir()) + sep + "teddy-canary-") ||
  ...
  if (
    ...
    realpathSync(join(root, "data")) !== join(root, "data")
  )
  ...
  try {
    ...
    await createHousehold(db, {
      name: CANARI_PROFILE_NAME,
      avatar: CANARI_PROFILE_AVATAR,
      childPin: CANARI_PROFILE_PIN,
      parentPin: "9292",
    });
  } finally {
    db.$client.close();
  ...
  seedRealWorldFixture({
    databasePath,
    publicDir: "public/generated/socle/canary",
    assetNamespace: "socle/canary",
    logPrefix: "isolated-canary",
  });
  const verify = createDatabase(databasePath);
  try {
    const counts = Object.fromEntries(
      ["profiles", "sessions", "progress", "attempts", "collection"].map((table) => [
        table,
        verify.$client.prepare(`SELECT COUNT(*) AS n FROM ${table}`).get(),
      ]),
    );
    writeFileSync(
      ...
      JSON.stringify(
        {
          profileId,
          counts,
          integrity: verify.$client.pragma("integrity_check", { simple: true }),
        },
        null,
        2,
      ),
