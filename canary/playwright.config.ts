...
if (process.env.TEDDY_CANARY_ROOT !== root || marker.root !== root || marker.databasePath !== process.env.DATABASE_PATH)
  throw new Error("Utiliser node scripts/run-isolated-canary.mjs depuis app/.");
const port = 33217;
const baseURL = `http://127.0.0.1:${port}`;
export default defineConfig({
  testDir: ".", testMatch: "teddy.spec.ts", workers: 1, fullyParallel: false, retries: 0,
  timeout: 180_000, expect: { timeout: 20_000 },
  outputDir: join(root, "test-results"), reporter: [["list"], ["json", { outputFile: join(root, "canary-results.json") }]],
  ...
  webServer: {
    ...
    cwd: root, gracefulShutdown: { signal: "SIGTERM", timeout: 10_000 },
  },
});
