...
6. **QA + modération kid-safe** avant qu'un monde ne devienne **jouable** : rejeter (et régénérer) tout asset effrayant/incohérent/avec texte.
   ...
   - **Purge des rejets** : les assets servables d'un monde **définitivement rejeté** (job `failed`, aucun retry ni succès) sont supprimés par une purge idempotente (`purgeFailedWorldAssets`, #176) → pas d'accumulation disque. Les mondes `buffered` (QA passée, en attente d'approbation) et `active` restent **intacts**.
