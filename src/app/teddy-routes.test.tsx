...
const m = vi.hoisted(() => ({ profile: vi.fn(), rest: vi.fn(), path: vi.fn(), household: vi.fn(), session: vi.fn(), profiles: vi.fn(), evolution: vi.fn(), selector: vi.fn(), screen: vi.fn() }));
...
vi.mock("@/components/ForestHome", () => ({ ForestHome: ({ children, quiet }: { children: ReactNode; quiet?: boolean }) => <div data-testid="home" data-quiet={quiet}>{children}</div> }));
vi.mock("@/components/ProfileSelector", () => ({ ProfileSelector: (props: unknown) => { m.selector(props); return <div>selector</div>; } }));
vi.mock("./(app)/collection/evolution-actions", () => ({ evolutionStateAction: m.evolution }));
vi.mock("@/components/game/EvolutionScreen", () => ({ EvolutionScreen: (props: unknown) => { m.screen(props); return <div>evolution</div>; } }));
vi.mock("@/components/game/ShardShopScreen", () => ({ ShardShopScreen: () => <div>shards</div> }));
vi.mock("@/components/game/WorldScenesPreview", () => ({ WorldScenesPreview: () => <div>preview</div> }));
beforeEach(() => { vi.clearAllMocks(); m.profile.mockResolvedValue(7); m.household.mockReturnValue(true); m.session.mockResolvedValue(null); });
afterEach(() => vi.unstubAllEnvs());
it.each([RestPage, ReturnPage])("redirects anonymous children before reading progression", async (page) => {
  m.profile.mockResolvedValue(null);
  await expect(page()).rejects.toThrow("redirect:/");
  expect(m.rest).not.toHaveBeenCalled(); expect(m.path).not.toHaveBeenCalled();
});
it.each([
...
])("renders rest reason %s, active=%s with the appropriate return path", async (reason, active, title, hint) => {
  ...
  } else expect(screen.getByRole("link", { name: active ? daily.resume : daily.continue })).toHaveAttribute("href", active ? "/jouer" : "/carte");
});
...
it("redirects parent login before listing profiles if onboarding is absent or session already exists", async () => {
  ...
  m.household.mockReturnValue(true); m.session.mockResolvedValue({ kind: "parent" });
...
it("links access settings to child profiles and parent recovery", () => {
  ...
  expect(screen.getByRole("link", { name: p.recover })).toHaveAttribute("href", "/parent/recuperation");
});
it("refuses invalid encoded creature IDs before requesting evolution state", async () => {
  await expect(EvolutionPage({ params: Promise.resolve({ id: "%zz" }) })).rejects.toThrow("redirect:/collection");
...
it("redirects missing evolution state, and passes authorized state with a decoded ID", async () => {
  ...
  await expect(EvolutionPage({ params: Promise.resolve({ id: "creature%3A0%3A0" }) })).rejects.toThrow("redirect:/collection");
...
it("renders the shard shop entry", () => { render(<ShardShopPage />); expect(screen.getByText("shards")).toBeInTheDocument(); });
it.each([["production", "", false], ["test", "worlds", true], ["development", "", true]])("guards the art workbench in %s with build flag %s", (env, flag, allowed) => {
  vi.stubEnv("NODE_ENV", env); vi.stubEnv("TEDDY_CHECK_BUILD", flag);
  if (allowed) { render(WorldScenesPage()); expect(screen.getByText("preview")).toBeInTheDocument(); }
