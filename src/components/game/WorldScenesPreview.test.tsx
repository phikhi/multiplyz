...
vi.mock("./ForestScene", () => ({ ForestScene: (props: unknown) => { mocks.scene(props); return null; } }));
vi.mock("./ForestMap", () => ({ ForestMap: (props: unknown) => { mocks.map(props); return <div data-testid="map" />; } }));
vi.mock("@/lib/sound/use-prefers-reduced-motion", () => ({ usePrefersReducedMotion: () => mocks.reduced }));
beforeEach(() => { vi.clearAllMocks(); mocks.reduced = false; });
it("previews arrival, progress and finale with the selected world's copy", () => {
  ...
  expect(mocks.scene).toHaveBeenLastCalledWith(expect.objectContaining({ phase: "arrival", completed: 0, showFriend: false }));
  ...
  expect(mocks.scene).toHaveBeenLastCalledWith(expect.objectContaining({ phase: "question", completed: 5 }));
  ...
  expect(mocks.scene).toHaveBeenLastCalledWith(expect.objectContaining({ phase: "finale", completed: 10 }));
...
it("keeps maps inert and propagates pause and reduced motion to both views", () => {
  ...
  expect(mocks.scene).toHaveBeenLastCalledWith(expect.objectContaining({ paused: true, reduced: true }));
  ...
  expect(mocks.map).toHaveBeenLastCalledWith(expect.objectContaining({ adventure: null, motion: { paused: true, reduced: true } }));
  fireEvent.click(screen.getByRole("button", { name: p.resume }));
  fireEvent.click(screen.getByRole("button", { name: p.reduced }));
  expect(mocks.map).toHaveBeenLastCalledWith(expect.objectContaining({ motion: { paused: false, reduced: false } }));
});
