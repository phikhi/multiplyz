...
const m = vi.hoisted(() => ({
  ...
  replace: vi.fn(),
  playSfx: vi.fn(),
  sound: vi.fn(async (_enabled: boolean) => {}),
  scene: vi.fn(),
  state: vi.fn(),
}));
...
it.each([
  [null, forest.loading],
  ["EMPTY", strings.play.emptyLevel],
  ["NETWORK", forest.loadError],
  ["UNAUTHENTICATED", forest.authError],
  ["INVALID", forest.loadError],
  ["STALE", forest.loadError],
])("offers the appropriate recovery when initial load returns %s", async (error, title) => {
  state(null, error);
  render(<App />);
  expect(screen.getByRole("heading", { name: title! })).toBeInTheDocument();
  if (error === "UNAUTHENTICATED")
    expect(screen.getByRole("link", { name: forest.login })).toHaveAttribute("href", "/");
  else if (error) {
    const refresh = error === "INVALID" || error === "STALE";
    fireEvent.click(
      screen.getByRole("button", { name: refresh ? forest.refresh : forest.retryNetwork }),
    );
    expect(refresh ? m.refresh : m.retry).toHaveBeenCalledOnce();
  }
  await act(async () => {});
});
it.each(["NETWORK", "UNAUTHENTICATED", "STALE"])(
  "preserves recovery both during play and inside a paused %s session",
  async (error) => {
    state(base, error);
    ...
    if (error !== "UNAUTHENTICATED") {
      fireEvent.click(
        screen.getByRole("button", {
          name: error === "NETWORK" ? forest.retryNetwork : forest.refresh,
        }),
      );
      expect(error === "NETWORK" ? m.retry : m.refresh).toHaveBeenCalledOnce();
    }
    state({ ...base, paused: true }, error);
    view.rerender(<App />);
    ...
    if (error === "UNAUTHENTICATED")
      expect(dialog.getByRole("link", { name: forest.login })).toHaveAttribute("href", "/");
    else {
      fireEvent.click(
...
it.each([false, true])(
  "keeps sound and reduced-motion controls usable, storage denied=%s",
  async (denied) => {
    if (denied)
...
it.each([false, true])(
  "focuses feedback and plays its sound once per revision, accompanied=%s",
  async (isRetrying) => {
    const adventure = {
      ...base,
      phase: "feedback" as const,
      game: { ...base.game, current: { ...base.game.current, isRetrying } },
    };
    state(adventure);
    const view = render(<App />);
    expect(
      screen.getByRole("heading", { name: isRetrying ? forest.accompanied : forest.correct }),
    ).toHaveFocus();
    expect(m.playSfx).toHaveBeenCalledExactlyOnceWith("correct");
    view.rerender(<App />);
    expect(m.playSfx).toHaveBeenCalledOnce();
    fireEvent.click(screen.getByRole("button", { name: forest.next }));
    expect(m.dispatch).toHaveBeenLastCalledWith("next");
    state({ ...adventure, revision: 1 });
    view.rerender(<App />);
...
it("identifies recalibration and treats completed game progress as complete", async () => {
  state({
    ...base,
    diagnostic: { recalibration: true, responses: [] },
    game: { ...base.game, finished: true },
  });
  render(<App />);
