...
|---|---|---|---|---|
| 1 | **Fin de scope** (épic clos, plus aucune story) | §5 clôture, étape 6 | oui | `release` |
...
5. **Gate « Parcours d'acceptation bout-en-bout » (obligatoire, R0.2/#316, WORKFLOW §21)** : avant de clore, exécuter le **vrai parcours utilisateur** dans un vrai navigateur sur les **vrais assets** (jamais un fixture), en suivant le flow de la spec comme l'enfant, et produire un **playthrough narré** (captures analysées + verdict) confirmant que l'épic est conforme bout-en-bout (points nommés WORKFLOW §21.c : carte atteinte · Teddy visible · art créature réel · boucle pièces→dépense). Ce playthrough est un **artefact PERSISTÉ** (jamais jetable en contexte de conversation, sinon reçu invérifiable #164) : **committé dans `docs/playthroughs/<epic>.md` ET posté en commentaire sur l'issue épic** — condition matérielle de la signature. **game-design + PO signent ce playthrough** — c'est l'**artefact** qui rend le point 4 exécutable, pas une simple relecture. **De plus, le canari « état jouable » (E2E full-loop, WORKFLOW §21.d, issue #326), UNE FOIS LIVRÉ (sous R1 #317), doit être VERT à la clôture** (2ᵉ condition, indépendante du playthrough) — conditionnel à sa livraison, car il n'existe pas encore à la clôture de R0 lui-même (cf. §21.d). **Un épic ne se clôt jamais sans le playthrough persisté signé (point c) ; le canari vert s'y ajoute dès qu'il est livré.**
