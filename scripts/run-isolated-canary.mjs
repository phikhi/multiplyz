...
if (args.some((arg) => arg !== "--prepare-only")) throw new Error("Usage: node scripts/run-isolated-canary.mjs [--prepare-only]");
...
for (const entry of ["src", "public", "drizzle", "canary", "e2e/seed-canari.ts", "scripts/lib/seed-real-world-fixture.ts", "test-fixtures/world/socle-sample", "package.json", "pnpm-lock.yaml", "tsconfig.json", "next-env.d.ts", "next.config.ts", "postcss.config.mjs", "tokens.css"]) {
...
writeFileSync(join(root, ".teddy-canary.json"), JSON.stringify({ version: 1, root, databasePath: join(root, "data/canary.sqlite") }, null, 2), { flag: "wx", mode: 0o600 });
...
const env = { PATH: process.env.PATH, HOME: process.env.HOME, TMPDIR: process.env.TMPDIR,
  LANG: process.env.LANG, NEXT_TELEMETRY_DISABLED: "1", DATABASE_PATH: join(root, "data/canary.sqlite"),
...
async function run(argv) {
  ...
  try {
    const code = await new Promise((yes, no) => { child.once("error", no); child.once("exit", (code) => yes(code ?? 1)); });
    if (code !== 0) throw new Error(`Canari interrompu (code ${code}). Preuves conservées : ${root}`);
...
if (!args.includes("--prepare-only")) await run(["node_modules/@playwright/test/cli.js", "test", "--config=canary/playwright.config.ts"]);
